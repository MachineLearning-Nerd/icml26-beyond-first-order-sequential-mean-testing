...
def raw_rows():
    for path in sorted(HERE.glob("claim3-raw-part-*.csv")):
...
def compare_regenerated(failures: list[str]) -> None:
    candidates = [
        CANDIDATE.parent / ".openresearch" / "artifacts" / "claim_3" / "generated",
        CANDIDATE / ".generated" / "claim-3",
    ]
    for generated in candidates:
        if not generated.exists():
            continue
        for name in ("decomposition_metrics.csv", "anscombe_metrics.csv"):
            committed = (HERE / name).read_text(encoding="utf-8").splitlines()
            observed = (generated / name).read_text(encoding="utf-8").splitlines()
            if committed != observed:
                failures.append(f"committed {name} differs from regenerated output at {generated}")
        with gzip.open(generated / "claim3_raw.csv.gz", "rt", newline="", encoding="utf-8") as handle:
            regenerated = csv.DictReader(handle)
            for index, (committed, observed) in enumerate(zip(raw_rows(), regenerated, strict=True)):
                if tuple(committed.values()) != tuple(observed.values()):
                    failures.append(f"committed raw data differs from regenerated row {index}")
                    break
...
def main() -> int:
    ...
    for n in [500, 2000, 10000, 50000]:
        ...
        for path in range(5000):
            if any(
                values[smaller][path] > values[larger][path] + 2e-12
                for smaller, larger in zip(deltas[1:], deltas[:-1], strict=True)
            ):
                nested_failures += 1
    if nested_failures:
        failures.append(f"{nested_failures} paths violate nested-window monotonicity")
    if len(decomposition) != 40000 or raw_count != 140000:
        failures.append("raw record count mismatch")

    manifest = json.loads((HERE / "raw_parts_manifest.json").read_text(encoding="utf-8"))
    for part in manifest["parts"]:
        observed_hash = hashlib.sha256((HERE / part["path"]).read_bytes()).hexdigest()
