...
def raw_rows():
    for path in sorted(HERE.glob("single-run-ci-paths-part-*.csv")):
...
def compare_regenerated(failures: list[str]) -> None:
    candidates = [
        CANDIDATE.parent / ".openresearch" / "artifacts" / "claim_4" / "generated",
        CANDIDATE / ".generated" / "claim-4",
    ]
    for generated in candidates:
        if not generated.exists():
            continue
        if (HERE / "single_run_ci_metrics.csv").read_text(encoding="utf-8").splitlines() != (
            generated / "single_run_ci_metrics.csv"
        ).read_text(encoding="utf-8").splitlines():
            failures.append(f"committed metrics differ from regenerated output at {generated}")
        with gzip.open(generated / "single_run_ci_paths.csv.gz", "rt", newline="", encoding="utf-8") as handle:
            for index, (committed, observed) in enumerate(zip(raw_rows(), csv.DictReader(handle), strict=True)):
