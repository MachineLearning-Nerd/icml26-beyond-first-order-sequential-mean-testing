...
def main() -> int:
    ...
    candidate_verifier_path = ROOT / "space_candidate" / "evidence" / "claim-1" / "verify_claim.py"
    candidate_verifier = run_script(candidate_verifier_path) if candidate_verifier_path.exists() else None
    ...
    candidate_exit = candidate_verifier.returncode if candidate_verifier is not None else None
    passed = verifier.returncode == 0 and checker.returncode == 0 and candidate_exit in {None, 0}
    summary = {
        "claim": 1,
    ...
    if not passed:
        ...
        if candidate_verifier is not None:
            print(candidate_verifier.stdout)
        return 1
