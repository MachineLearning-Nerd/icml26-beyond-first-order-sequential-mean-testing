...
def run_claim(config: dict[str, Any], output_dir: Path, artifacts_root: Path, source_dir: Path) -> dict[str, Any]:
    ...
    claim1_dir = artifacts_root / "claim-1"
    claim2_dir = artifacts_root / "claim-2"
    claim1_verdict = json.loads((claim1_dir / "verifier_output.json").read_text(encoding="utf-8"))
    claim2_verdict = json.loads((claim2_dir / "verifier_output.json").read_text(encoding="utf-8"))
    with (claim2_dir / "stopping_clt_metrics.csv").open(newline="", encoding="utf-8") as handle:
